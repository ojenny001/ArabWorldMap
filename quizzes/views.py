...
from .models import Quiz, Question, Country
...
def progress_view(request, countryName):
    ...
    prequiz = Quiz.objects.filter(name__contains='Pre-Quiz')
    phase1 = Quiz.objects.filter(name__contains='Phase 1')
    phase2 = Quiz.objects.filter(name__contains='Phase 2')
    phase3 = Quiz.objects.filter(name__contains='Phase 3')
...
def quiz_view(request, countryName, pk):
    ...
    quiz = Quiz.objects.get(country=country, pk=pk)
